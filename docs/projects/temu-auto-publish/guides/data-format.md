...
| 关键词 | 文本 | ✓ | 站内搜索关键词 | "智能手表" |
| 实拍图数组 | JSON/文本 | ✗ | SKU/实拍图片文件名或 URL 数组 | `["A045.jpg","A046.jpg"]` |
...
- 所有必填字段不能为空
- 成本价保留 2 位小数
- 若缺少 `尺寸图链接`，首次编辑阶段将跳过尺寸图上传；系统不再尝试根据前缀自动拼接外链。
- 若 `视频链接` 留空，系统会读取 `VIDEO_BASE_URL` 环境变量并拼接型号编号（示例：`A026.mp4`）生成默认视频 URL。
- 若提供 `视频链接`，系统仍会优先使用 `VIDEO_BASE_URL` + 型号编号（如 `A026.mp4`）拼接的 OSS 地址；仅当未配置 `VIDEO_BASE_URL` 时才回退到表格中的 URL。
