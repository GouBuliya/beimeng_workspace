...
class TestResilientLocator:
    ...
    async def test_fill_no_clear(self, locator, mock_page):
        ...
        result = await locator.fill(
            mock_page, "title_input", "测试", clear_first=False
        )
    ...
    def test_suggest_optimizations_low_primary_rate(self, locator):
        ...
        reorder_suggestion = next(
            (s for s in suggestions if s["type"] == "reorder"),
            None
        )
    ...
    def test_suggest_optimizations_low_success_rate(self, locator):
        ...
        add_suggestion = next(
            (s for s in suggestions if s["type"] == "add_selectors"),
            None
        )
