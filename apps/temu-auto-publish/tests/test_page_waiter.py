...
class TestWaitStrategy:
    ...
    def test_custom_values(self):
        """测试自定义值"""
        strategy = WaitStrategy(
            wait_after_action_ms=100, wait_for_stability_timeout_ms=1000, retry_initial_delay_ms=50
        )
    ...
    def test_next_retry_delay_first_attempt(self):
        """测试第一次重试延迟"""
        strategy = WaitStrategy(
            retry_initial_delay_ms=100, retry_backoff_factor=2.0, retry_max_delay_ms=1000
        )
    ...
    def test_next_retry_delay_capped(self):
        """测试延迟上限"""
        strategy = WaitStrategy(
            retry_initial_delay_ms=100, retry_backoff_factor=2.0, retry_max_delay_ms=500
        )
...
class TestPageWaiter:
    ...
    def test_init_with_custom_strategy(self, mock_page):
        """测试使用自定义策略初始化"""
        custom_strategy = WaitStrategy(wait_after_action_ms=200, dom_stable_checks=5)
    ...
    async def test_post_action_wait(self, mock_page):
        ...
        await waiter.post_action_wait(wait_for_network_idle=True, wait_for_dom_stable=True)
    ...
    async def test_wait_for_dom_stable(self, mock_page):
        ...
        waiter = PageWaiter(
            mock_page, strategy=WaitStrategy(dom_stable_checks=3, dom_stable_interval_ms=10)
        )
    ...
    async def test_apply_retry_backoff(self, mock_page):
        ...
        waiter = PageWaiter(
            mock_page,
            strategy=WaitStrategy(
                retry_initial_delay_ms=10, retry_backoff_factor=2.0, retry_max_delay_ms=100
            ),
        )
...
class TestPageWaiterEdgeCases:
    ...
    async def test_zero_delay(self):
        """测试零延迟"""
        strategy = WaitStrategy(retry_initial_delay_ms=0, retry_max_delay_ms=0)
    ...
    async def test_negative_attempt(self):
        """测试负数尝试次数"""
        strategy = WaitStrategy(retry_initial_delay_ms=100, retry_backoff_factor=2.0)
    ...
    async def test_dom_stability_changes(self):
        ...
        waiter = PageWaiter(
            mock_page,
            strategy=WaitStrategy(
                dom_stable_checks=3, dom_stable_interval_ms=5, wait_for_stability_timeout_ms=50
            ),
        )
...
class TestWaitStrategyCalculations:
    ...
    def test_backoff_sequence(self):
        """测试退避序列"""
        strategy = WaitStrategy(
            retry_initial_delay_ms=100, retry_backoff_factor=1.5, retry_max_delay_ms=1000
        )
    ...
    def test_backoff_factor_effect(self):
        ...
        strategy_slow = WaitStrategy(
            retry_initial_delay_ms=100, retry_backoff_factor=1.5, retry_max_delay_ms=10000
        )
