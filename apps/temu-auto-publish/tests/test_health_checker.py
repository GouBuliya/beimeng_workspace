...
class TestHealthCheckResult:
    ...
    def test_create_result(self):
        ...
        result = HealthCheckResult(
            component="browser", status=HealthStatus.OK, message="Browser is running"
        )
    ...
    def test_is_healthy_ok(self):
        """测试OK状态是健康的"""
        result = HealthCheckResult(component="test", status=HealthStatus.OK, message="All good")

        assert result.is_healthy() is True

    def test_is_healthy_warning(self):
        """测试WARNING状态也是健康的"""
        result = HealthCheckResult(component="test", status=HealthStatus.WARNING, message="Warning")

        assert result.is_healthy() is True

    def test_is_healthy_error(self):
        """测试ERROR状态不健康"""
        result = HealthCheckResult(component="test", status=HealthStatus.ERROR, message="Error")

        assert result.is_healthy() is False

    def test_is_healthy_unknown(self):
        """测试UNKNOWN状态不健康"""
        result = HealthCheckResult(component="test", status=HealthStatus.UNKNOWN, message="Unknown")
...
class TestHealthChecker:
    ...
    async def test_check_all(self):
        ...
        # Mock各个检查方法
        checker.check_disk = AsyncMock(
            return_value=HealthCheckResult(
                component="disk", status=HealthStatus.OK, message="Disk OK"
            )
        )
        checker.check_memory = AsyncMock(
            return_value=HealthCheckResult(
                component="memory", status=HealthStatus.OK, message="Memory OK"
            )
        )
        checker.check_network = AsyncMock(
            return_value=HealthCheckResult(
                component="network", status=HealthStatus.OK, message="Network OK"
            )
        )
        checker.check_browser = AsyncMock(
            return_value=HealthCheckResult(
                component="browser", status=HealthStatus.WARNING, message="Browser not started"
            )
        )
    ...
    async def test_check_all_overall_status(self):
        ...
        # 全部OK
        checker.check_disk = AsyncMock(
            return_value=HealthCheckResult(component="disk", status=HealthStatus.OK, message="OK")
        )
        checker.check_memory = AsyncMock(
            return_value=HealthCheckResult(component="memory", status=HealthStatus.OK, message="OK")
        )
        checker.check_network = AsyncMock(
            return_value=HealthCheckResult(
...
class TestHealthCheckerIntegration:
    ...
    async def test_real_disk_check(self):
        ...
        assert (
            "free_gb" in result.details or "error" in result.message.lower() or result.details == {}
        )
