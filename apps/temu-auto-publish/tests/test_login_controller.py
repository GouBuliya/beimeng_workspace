...
class TestLoginControllerLoginFlow:
    ...
    async def test_login_with_valid_credentials(self, mock_controller):
        ...
        with patch.object(mock_controller.browser_manager, 'start', new_callable=AsyncMock):
            with patch.object(mock_controller.browser_manager, 'goto', new_callable=AsyncMock):
                with patch.object(mock_controller.browser_manager, 'save_cookies', new_callable=AsyncMock):
                    # Mock page 操作
                    mock_controller.browser_manager.page.wait_for_selector = AsyncMock(return_value=MockLocator())
    ...
    async def test_login_if_needed_already_logged_in(self, mock_controller):
        ...
        with patch.object(mock_controller.browser_manager, 'start', new_callable=AsyncMock):
            with patch.object(mock_controller.browser_manager, 'load_cookies', new_callable=AsyncMock, return_value=True):
                with patch.object(mock_controller.browser_manager, 'goto', new_callable=AsyncMock):
...
class TestLoginControllerCookieLogin:
    ...
    async def test_cookie_login_success(self, controller_with_mocks):
        ...
        with patch.object(controller.browser_manager, 'start', new_callable=AsyncMock):
            with patch.object(controller.browser_manager, 'load_cookies', new_callable=AsyncMock, return_value=True):
    ...
    async def test_cookie_expired_fallback_to_login(self, controller_with_mocks):
        ...
        controller._check_login_status = AsyncMock(side_effect=[False, True])  # 第一次失败，第二次成功
        
        with patch.object(controller.browser_manager, 'start', new_callable=AsyncMock):
            with patch.object(controller.browser_manager, 'load_cookies', new_callable=AsyncMock, return_value=True):
                with patch.object(controller.browser_manager, 'goto', new_callable=AsyncMock):
                    with patch.object(controller.browser_manager, 'save_cookies', new_callable=AsyncMock):
                        controller.browser_manager.page.wait_for_selector = AsyncMock(return_value=MockLocator())
...
class TestLoginControllerCheckStatus:
    ...
    def controller(self, tmp_path):
        ...
        selector_file.write_text(json.dumps({
            ...
            "homepage": {
                "success_indicators": [".jx-main", ".pro-layout"]
            }
        }))
