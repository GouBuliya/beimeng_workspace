...
class TestProductSelectionRow:
    ...
    def test_model_number_validation_auto_prefix(self):
        """测试型号编号自动添加A前缀"""
        row = ProductSelectionRow(product_name="test", model_number="0001")

        assert row.model_number == "A0001"

    def test_model_number_validation_empty(self):
        """测试空型号编号"""
        row = ProductSelectionRow(product_name="test", model_number="")
    ...
    def test_spec_options(self):
        """测试规格选项"""
        row = ProductSelectionRow(
            product_name="test", spec_options=["小号", "中号", "大号"], spec_unit="尺寸"
        )
    ...
    def test_variant_costs(self):
        """测试多规格进货价"""
        row = ProductSelectionRow(
            product_name="test", cost_price=10.0, variant_costs=[10.0, 15.0, 20.0]
        )
    ...
    def test_sku_image_urls(self):
        ...
        row = ProductSelectionRow(
            product_name="test",
            sku_image_urls=["https://example.com/img1.jpg", "https://example.com/img2.jpg"],
        )
...
class TestSelectionTableReaderValidation:
    ...
    def test_validate_row_success(self, reader):
        """测试验证成功"""
        row = {"product_name": "药箱收纳盒", "model_number": "A0001"}
