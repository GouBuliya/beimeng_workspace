...
class TestCollectionWorkflowExecution:
    ...
    async def test_workflow_execution_failure(self, mock_page):
        ...
        try:
            ...
            with patch.object(
                workflow, "execute", return_value={"success": False, "error": "Test error"}
            ):
...
class TestCollectionWorkflowData:
    ...
    def test_product_data_structure(self):
        """测试产品数据结构"""
        product_data = {"keyword": "药箱收纳盒", "model_number": "A0001", "collect_count": 5}
    ...
    def test_multiple_products(self):
        """测试多个产品"""
        products = [{"keyword": f"产品{i}", "model_number": f"A{i:04d}"} for i in range(1, 6)]
...
class TestCollectionWorkflowConfig:
    ...
    def test_default_config(self):
        """测试默认配置"""
        config = {"timeout_ms": 30000, "max_collect_count": 5, "retry_count": 3}
    ...
    def test_custom_config(self):
        """测试自定义配置"""
        config = {"timeout_ms": 60000, "max_collect_count": 10, "retry_count": 5}
