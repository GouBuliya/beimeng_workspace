...
async def demo_login():
    ...
    console.print(Panel.fit(
        "[bold cyan]演示1: 自动登录妙手ERP[/bold cyan]",
        border_style="cyan"
    ))
...
async def demo_navigation(page):
    """演示导航和产品统计功能."""
    console.print(Panel.fit(
        "[bold cyan]演示2: 导航到公用采集箱 & 获取产品统计[/bold cyan]",
        border_style="cyan"
    ))
...
async def demo_data_processing():
    """演示数据处理功能."""
    console.print(Panel.fit(
        "[bold cyan]演示3: 价格计算 & 随机数据生成[/bold cyan]",
        border_style="cyan"
    ))
...
async def demo_search_and_first_edit(page):
    """演示搜索和查看产品详情."""
    console.print(Panel.fit(
        "[bold cyan]演示4: 产品列表查看[/bold cyan]",
        border_style="cyan"
    ))
    ...
    console.print("[dim]说明: SearchController可以按关键词搜索商品，但演示中不执行以避免修改数据[/dim]\n")


async def demo_batch_edit_preview(page):
    """演示批量编辑选择器（预览，不实际执行）."""
    console.print(Panel.fit(
        "[bold cyan]演示5: 批量编辑18步流程（架构预览）[/bold cyan]",
        border_style="cyan"
    ))
...
async def demo_cookie_management():
    """演示Cookie管理功能."""
    console.print(Panel.fit(
        "[bold cyan]演示6: Cookie管理系统[/bold cyan]",
        border_style="cyan"
    ))
