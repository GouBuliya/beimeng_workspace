    ...
    def generate_report(self) -> BenchmarkSuite:
        ...
        suite = BenchmarkSuite(
            ...
            summary={
                ...
                "fastest_benchmark": min(self.results, key=lambda r: r.avg_time_ms).name
                if self.results
                else None,
                "slowest_benchmark": max(self.results, key=lambda r: r.avg_time_ms).name
                if self.results
                else None,
            },
        )
...
async def benchmark_enhanced_retry(runner: BenchmarkRunner) -> None:
    ...
    handler2 = EnhancedRetryHandler(RetryPolicy(max_attempts=5, initial_delay_ms=1, jitter=False))
...
async def benchmark_comparison(runner: BenchmarkRunner) -> None:
    ...
    handler = EnhancedRetryHandler(RetryPolicy(max_attempts=3, initial_delay_ms=1, jitter=False))
...
async def main():
    ...
    parser.add_argument(
        "-i", "--iterations", type=int, default=100, help="每个测试的迭代次数 (默认: 100)"
    )
    parser.add_argument("-w", "--warmup", type=int, default=10, help="预热迭代次数 (默认: 10)")
