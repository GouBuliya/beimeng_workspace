...
def main():
    """主入口函数."""
    parser = argparse.ArgumentParser(description="Temu自动发布系统 - 定时任务守护进程")

    subparsers = parser.add_subparsers(dest="command", help="命令")

    # start命令
    start_parser = subparsers.add_parser("start", help="启动守护进程")
    start_parser.add_argument("--config", type=Path, help="配置文件路径")
    start_parser.add_argument("--daemon", action="store_true", help="以守护进程模式运行(后台运行)")
