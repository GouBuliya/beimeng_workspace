...
class CompletePublishWorkflow:
    ...
    async def execute_full_workflow(
        ...
        try:
            ...
            stage1_result = await self.stage1_first_edit(
                product_data_list[: self.FIRST_EDIT_COUNT], username
            )
    ...
    async def stage1_first_edit(
        ...
        Returns:
            阶段执行结果
        """
        result = {"success": False, "edited_count": 0, "edited_products": [], "message": ""}
        ...
        try:
            ...
            if username:
                ...
                try:
                    ...
                    # 选择用户
                    user_option = self.page.locator(
                        f".jx-select-dropdown__item:has-text('{username}')"
                    ).first
            ...
            for i, product_data in enumerate(product_data_list):
                try:
                    ...
                    result["edited_products"].append(
                        {"index": i, "product_id": product_data.get("id", f"product_{i}")}
                    )
    ...
    async def stage3_batch_edit(self, product_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            阶段执行结果
        """
        result = {"success": False, "success_count": 0, "failed_count": 0, "message": ""}
        ...
        try:
                ...
                try:
                    ...
                    if await set_price_btn.count() > 0:
                        ...
                        price_input = self.page.locator(
                            "input[type='number'], input[placeholder*='价格']"
                        ).first
            ...
            try:
                ...
                if await publish_btn.count() > 0:
                    ...
                    # 第2次确认
                    confirm_publish_btn = self.page.locator(
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        if await login_ctrl.login(username, password, headless=False):
            ...
            # 执行完整工作流
            workflow = CompletePublishWorkflow(page)
            result = await workflow.execute_full_workflow(product_data_list, username="keshijun123")
