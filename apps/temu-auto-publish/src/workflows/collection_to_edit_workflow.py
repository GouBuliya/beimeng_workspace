...
class CollectionToEditWorkflow:
    ...
    def __init__(
        ...
        self.five_to_twenty = FiveToTwentyWorkflow(
            use_ai_titles=use_ai_titles, debug_mode=debug_mode
        )
        ...
        if output_dir:
            ...
            self.output_dir = (
                Path(__file__).parent.parent.parent / "data" / "output" / "collection_to_edit"
            )
    ...
    async def execute(
        ...
        try:
            ...
            stage3_result = await self._stage_navigate_to_collection_box(
                page, filter_by_user=filter_by_user
            )
        ...
        try:
            ...
            try:
                ...
                for i, product in enumerate(products):
                    ...
                    try:
                        # 步骤2: 搜索商品
                        if not await self.collection_ctrl.search_products(
                            temu_page, product.product_name
                        ):
                        ...
                        links = await self.collection_ctrl.collect_links(
                            temu_page, count=product.collect_count
                        )
    ...
    async def _stage_add_to_miaoshou(self, page: Page, product_urls: list[str]) -> dict:
        ...
        try:
            result = await self.collection_ctrl.add_to_miaoshou_collection_box(
                page, product_urls, max_retries=3, use_plugin=True
            )
    ...
    async def _stage_navigate_to_collection_box(
        ...
        try:
            success = await self.miaoshou_ctrl.navigate_and_filter_collection_box(
                page, filter_by_user=filter_by_user, switch_to_tab="all"
            )
        ...
        try:
            ...
            if products_data:
                ...
                for i, pd in enumerate(products_data, 1):
                    ...
                    logger.info(
                        f"  产品{i}: {pd['keyword']} ({pd['model_number']}) - 成本{cost_str}"
                    )
                logger.info("")
