...
class StateDetector:
    ...
    async def is_login_page(self, page: Page) -> bool:
        ...
        try:
            ...
            # 检查是否有登录表单
            login_btn_count = await page.locator("button:has-text('登录'), button:has-text('立即登录')").count()
    ...
    async def ensure_state(
        ...
        if not auto_recover:
            logger.warning(f"⚠️ 当前状态({current_state.value}) != 期望状态({expected_state.value})，且未启用自动恢复")
            return False
        
        # 尝试恢复
        logger.warning(f"⚠️ 当前状态({current_state.value}) != 期望状态({expected_state.value})，尝试恢复...")
