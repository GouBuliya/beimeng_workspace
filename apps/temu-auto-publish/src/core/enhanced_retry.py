    ...
    async def execute(
        ...
        **kwargs: P.kwargs,
        ...
        for attempt in range(1, self.policy.max_attempts + 1):
            ...
            try:
                logger.debug(f"执行 {func.__name__} (尝试 {attempt}/{self.policy.max_attempts})")
                ...
                if attempt > 1:
                    logger.success(f"✓ {func.__name__} 重试成功 (第{attempt}次尝试)")
                ...
                if not self.policy.is_retryable(exc, attempt):
                    if attempt >= self.policy.max_attempts:
                        logger.error(f"✗ {func.__name__} 重试次数耗尽 ({attempt}次)")
                    else:
