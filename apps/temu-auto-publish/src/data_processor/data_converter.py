...
class DataConverter:
    ...
    def selection_to_collection(
        ...
        for product in products:
            ...
            logger.debug(f"  - {product.product_name} ({product.model_number}): 采集{product.collect_count}个")
    ...
    def collection_to_edit(
        ...
        selection_map = {
            ...
            for product in selection_products
        }
        ...
        for i, result in enumerate(collection_results):
            ...
            logger.debug(
                f"  - 产品 {i+1}: {product_name} ({model_number})"
                f" - {links_count} 个链接"
            )
    ...
    def validate_collection_results(
        ...
        if len(results) < expected_count:
            ...
            validation["issues"].append(
                f"产品数量不足: {len(results)} < {expected_count}"
            )
    ...
    def merge_collection_and_selection(
        ...
        for result in collection_results:
            ...
            if selection_product:
                ...
                logger.debug(
                    f"  - {product_name}: {merged['collected_count']} 个链接"
                )
            else:
