...
class AICategoryAttrFiller:
    ...
    # LLM 提示词模板
    PROMPT_TEMPLATE = """你是一个电商产品属性专家。根据商品信息推断以下必填属性的值。
...
重要规则：
1. 每个属性必须从可选项中选择一个值，不能返回"否"、"不适用"等非选项值
2. 根据商品标题推断最合适的选项，如标题含"不锈钢"则材质选"不锈钢"
3. 对于与商品无关的属性（如非木制品的"木种"），选择列表中第一个选项即可
4. 电源相关属性：若商品不带电，电源方式选"不带电"，工作电压选最低电压选项
5. 如果无法确定，选择最通用/常见的选项

请以 JSON 格式返回，格式如下：
```json
[
  {{"attrName": "属性名1", "attrValue": "选择的值1"}},
  {{"attrName": "属性名2", "attrValue": "选择的值2"}}
]
```
